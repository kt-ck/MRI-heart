...
function Main({ width, height }) {
  ...
  // const dicom_list = useSelector((state) => state.global.dicom_list);
        ...
        if (begin[0] !== e.clientX || begin[1] !== e.clientY) {
          ...
          if (e.which === 1) {
            ...
            ctx.strokeRect(
              x - x_del,
              y - y_del,
              Math.abs(begin[0] - e.clientX),
              Math.abs(begin[1] - e.clientY)
            );
            console.log(
              dicomInfo.PhysicalDelta[0] * Math.abs(begin[0] - e.clientX),
